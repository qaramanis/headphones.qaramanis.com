import { getLastPlayed, saveLastPlayed } from "@/lib/kv";
...
export async function GET(request: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (initialTrack) {
        // Save to Redis on initial load
        await saveLastPlayed(initialTrack);
        console.log(
          `[Stream] Initial track saved to Redis: ${initialTrack.title} by ${initialTrack.artist} (isPlaying: ${initialTrack.isPlaying})`
        );
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (currentTrack) {
            // Compare with previous track
            const trackChanged =
              !previousTrack ||
              previousTrack.trackId !== currentTrack.trackId ||
              previousTrack.isPlaying !== currentTrack.isPlaying;

            if (trackChanged) {
              // Save to Redis only when track changes
              await saveLastPlayed(currentTrack);
              console.log(
                `[Stream] Track changed, saved to Redis: ${currentTrack.title} by ${currentTrack.artist} (isPlaying: ${currentTrack.isPlaying})`
              );
